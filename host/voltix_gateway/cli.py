...
import voltix_gateway.server
from voltix_gateway import Transceiver
import time
import signal
import sys
import json
...
def server(ctx, device, port, host):
    voltix_gateway.server.tcv = Transceiver(port=device)
    uvicorn.run("voltix_gateway.server:app", port=port, host=host)
